...
const CartPage = () => {
  ...
  const { items, loading } = useSelector((state) => state.fetchCart);
  ...
  const subtotal =
    items?.reduce((sum, item) => sum + item.unit_price * item.quantity, 0) || 0;
          ...
          {items?.map((item) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <div className="font-bold 2xl:text-3xl">
                    {item.product?.name}
                  </div>
              ...
              <div className="font-semibold 2xl:text-2xl">${item.total}</div>
              <div>
                <button
                  onClick={() => handleDelete(item.id)}
                  className="bg-[#ff0800] text-white px-3 py-2 rounded shadow cursor-pointer hover:bg-[#ff0800c7]"
                >
          ...
          {items?.length > 0 && (
            <div className="flex justify-end px-4 py-4">
              <button
                ...
                className="text-[#ff0800] border border-[#ff0800] px-4 py-2 rounded hover:bg-[#ff0800] hover:text-white transition"
              >
                Clear All Cart
              </button>
            </div>
